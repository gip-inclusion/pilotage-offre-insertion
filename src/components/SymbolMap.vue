    ...
    updateMarkers() {
      ...
      if (listOfServices && listOfServices.length > 0 && Array.isArray(listOfServices)) {
        listOfServices.forEach(item => {
          ...
          if (item.Thematiques && item.Latitude && item.Longitude && 
              (!this.selectedThematique || thematiques.some(t => t.includes(this.selectedThematique)) ) && !this.servicesBlacklist.includes(item["ID"])) {
                if(this.selectedDepartement.length == 3 || this.selectedDepartement.length == 2 && item.Longitude < 10 ){
